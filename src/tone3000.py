...
def _safe_filename(value, fallback):
    """Keep remote model names inside the destination directory."""
...
def download(tone_id, dest_dir, tag=None, a2_only=True, ext=None,
    ...
    for i, m in enumerate(sorted(ms, key=lambda x: x["id"]), 1):
        ...
        if not fname:
            ...
            fallback = Path(urllib.parse.unquote(url_path)).name
        fname = _safe_filename(fname, fallback or f"model-{m['id']}")
        ...
        if ext:
            ...
            fname = f"{fname}.nam"
        out = (dest_root / fname).resolve()
        if out.parent != dest_root:
            raise ValueError(f"unsafe download destination: {fname!r}")
        if progress:
