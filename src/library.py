...
# TONE3000 search fields (minus search-level `total_count`) plus local columns.
# ``platform`` remains only as a migration/read compatibility alias; new rows
# use the official ``format`` field.
TONE_COLUMNS = [
...
CREATE TABLE IF NOT EXISTS tones (
    ...
    format              TEXT,          -- canonical TONE3000 Format enum
    platform            TEXT,          -- deprecated alias, retained for old DBs
    sizes               TEXT,          -- JSON array of TONE3000 Size values
    license             TEXT,          -- TONE3000 License enum
    links               TEXT,          -- JSON array of creator links
...
CREATE TABLE IF NOT EXISTS models (
    ...
    size         TEXT,          -- TONE3000 Size enum
    architecture_version TEXT,  -- canonical Architecture: 1 / 2 / custom / NULL
    architecture TEXT,
    local_path   TEXT
);
...
def connect() -> sqlite3.Connection:
    ...
    for column, definition in (
            ("format", "TEXT"), ("sizes", "TEXT"), ("license", "TEXT"),
            ("links", "TEXT"), ("user", "TEXT"), ("user_url", "TEXT"),
            ("is_public", "INTEGER"), ("url", "TEXT")):
        if column not in tone_columns:
            conn.execute(f"ALTER TABLE tones ADD COLUMN {column} {definition}")
    model_columns = {
        row[1] for row in conn.execute("PRAGMA table_info(models)").fetchall()
    }
    for column, definition in (
            ("created_at", "TEXT"), ("updated_at", "TEXT"),
            ("user_id", "TEXT"), ("size", "TEXT")):
        if column not in model_columns:
            conn.execute(f"ALTER TABLE models ADD COLUMN {column} {definition}")
    # One-time additive backfill. Keep the deprecated aliases intact so older
    # callers can still read the database while canonical code uses the new
    # columns.
    if "platform" in tone_columns:
        conn.execute("UPDATE tones SET format = lower(platform) "
                     "WHERE format IS NULL AND platform IS NOT NULL")
    # Normalize aliases and recover legacy IR rows after the canonical format
    # column is backfilled.  An explicit platform/format still wins over the
    # old cab/space inference.
    conn.execute("UPDATE tones SET gear = 'amp-cab' "
                 "WHERE lower(gear) = 'full-rig'")
    conn.execute("UPDATE tones SET format = 'ir' "
                 "WHERE format IS NULL AND lower(gear) IN ('cab', 'space', 'ir')")
    conn.execute("UPDATE tones SET gear = 'cab' WHERE lower(gear) = 'ir'")
    conn.execute(
        "UPDATE models SET architecture_version = CASE lower(architecture) "
        "WHEN '1' THEN '1' WHEN 'a1' THEN '1' WHEN 'wavenet' THEN '1' "
        "WHEN '2' THEN '2' WHEN 'a2' THEN '2' "
        "WHEN 'slimmablecontainer' THEN '2' WHEN 'custom' THEN 'custom' "
        "ELSE architecture_version END "
        "WHERE architecture_version IS NULL AND architecture IS NOT NULL")
    conn.commit()
    return conn
...
def upsert_model(conn: sqlite3.Connection, m: dict, *, commit: bool = True) -> None:
    ...
    """
    m = tone3000.normalize_model(m)
    if m.get("local_path"):
        m["local_path"] = _to_rel_path(m["local_path"])
    sql = ("INSERT INTO models (id, tone_id, created_at, updated_at, user_id, "
           "model_url, name, size, architecture_version, architecture, local_path) "
           "VALUES (:id, :tone_id, :created_at, :updated_at, :user_id, "
           ":model_url, :name, :size, :architecture_version, :architecture, "
           ":local_path) "
...
def list_local_models(kind: str = "amp", limit: int = 2000) -> list[dict]:
    ...
    """
    # The current engine has exactly two processing paths.  Preserve metadata
    # for newer TONE3000 formats, but never expose those files as a loadable
    # NAM/IR model in the picker.
    arch = ("lower(m.local_path) LIKE '%.wav' AND "
            "(upper(m.architecture) = 'IR' OR "
            "COALESCE(t.format, 'nam') IN ('nam', 'ir'))"
            if kind == "ir" else
            "(COALESCE(t.format, 'nam') = 'nam' AND "
            "COALESCE(upper(m.architecture), '') <> 'IR' AND "
            "lower(m.local_path) LIKE '%.nam')")
    with connect() as conn:
        rows = conn.execute(
            f"SELECT m.id, m.tone_id, m.created_at, m.updated_at, m.user_id, "
            "m.model_url, m.name, m.size, m.architecture_version, m.architecture, "
            "m.local_path, t.title, t.username, t.gear, t.format, t.sizes, "
            "t.license, t.links, t.user, t.user_url, t.is_public, t.url, "
            "t.description, "
            "t.tags, t.makes "
...
def import_tone(tone_id: int, progress=None, *, quiet: bool = False,
    ...
    download). IR tones (format=ir, including legacy cab/space rows) are
    recorded with a null architecture_version and architecture="IR".
    """
    row = tone3000.tone_by_id(tone_id)
    if not row:
        if not quiet:
    ...
    raw_row = dict(row)
    row = tone3000.normalize_tone(row)
    fmt = tone3000.tone_format(row)
    if fmt is not None and fmt not in tone3000.ENGINE_FORMATS:
        if not quiet:
            print(f"TONE3000 format {fmt} is not supported by the current engine.")
        return None
    is_ir = tone3000.is_ir_tone(row)
    # Pre-v1 callers sometimes supplied a stale ``platform=nam`` alongside a
    # cab/space gear token. Canonical ``format`` always wins; this fallback is
    # limited to rows with neither canonical nor legacy format metadata and
    # keeps old imported IRs usable during migration.
    if (raw_row.get("format") in (None, "")
            and raw_row.get("platform") in (None, "")
            and raw_row.get("gear") in {"cab", "space"}):
        is_ir = True
    slug = tone3000.slugify(row.get("title"), 40)
    dest = TONES_DIR / f"{tone_id}-{slug}"
    paths = tone3000.download(tone_id, dest, tag=slug,
                              # The pack picker exposes every applicable
                              # architecture.  ``model_ids`` still limits a
                              # partial install, so no incompatible file is
                              # downloaded when the user selects a subset.
                              a2_only=False,
    ...
    with connect() as conn:
        ...
        for m in paths:
            upsert_model(conn, {
                ...
                "size": m.get("size"),
                "architecture_version": m.get("architecture_version"),
                "architecture": (m["model_json"] or {}).get("architecture") or
                ("IR" if is_ir or str(m.get("local_path") or "").lower().endswith(".wav")
                 else None),
...
def _first_local_model(tone_id: int, ir: bool = False) -> int | None:
    ...
    with connect() as conn:
        if ir:
            sql = ("SELECT m.id FROM models m JOIN tones t ON t.id = m.tone_id "
                   "WHERE m.tone_id = ? AND m.local_path IS NOT NULL "
                   "AND lower(m.local_path) LIKE '%.wav' "
                   "AND (upper(m.architecture) = 'IR' OR "
                   "COALESCE(t.format, 'nam') IN ('nam', 'ir')) "
                   "ORDER BY m.id LIMIT 1")
            params = (tone_id,)
        else:
            # Prefer A2 for the existing quality-control path, then fall back
            # to A1/Custom so an A1-only pack remains loadable.
            sql = ("SELECT m.id FROM models m JOIN tones t ON t.id = m.tone_id "
                   "WHERE m.tone_id = ? AND m.local_path IS NOT NULL "
                   "AND COALESCE(t.format, 'nam') = 'nam' "
                   "AND COALESCE(upper(m.architecture), '') <> 'IR' "
                   "AND lower(m.local_path) LIKE '%.nam' "
                   "ORDER BY CASE architecture_version WHEN '2' THEN 0 "
                   "WHEN '1' THEN 1 ELSE 2 END, id LIMIT 1")
            params = (tone_id,)
...
def local_models_by_tone(path: str) -> list[dict] | None:
    ...
    with connect() as conn:
        ...
        if not row:
            return None
        rows = conn.execute(
            "SELECT id, tone_id, created_at, updated_at, user_id, name, size, "
            "architecture_version, architecture, model_url, local_path FROM models "
...
def mark_download_state(hits: list[dict]) -> list[dict]:
    ...
    id-by-id against TONE3000's current model list across every
    architecture/format, queried in parallel. The pack picker and importer now
    expose the same complete model set, so an A1-only or Custom-only pack
...
def _fmt_table(tones: list[dict]) -> str:
    rows = [
        ...
        f"a2={t.get('a2_models_count', 0):>3} | "
        f"{tone3000.normalize_gear(t.get('gear')) or '?':<8} | "
        f"{tone3000.tone_format(t) or '?':<11} | "
...
def _fmt_show(t: dict) -> str:
    lines = [
        ...
        f"title        {t.get('title')}",
        f"gear         {t.get('gear')}",
        f"format       {tone3000.tone_format(t)}",
    ...
    if ms:
        ...
        for m in ms:
            ...
            version = tone3000.model_architecture_version(m) or "-"
            lines.append(f"  {m['id']} [{arch}; v{version}] "
                         f"{m.get('local_path') or m.get('model_url')}")
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    pl.add_argument("--gear", choices=list(tone3000.GEAR_VALUES), help="filter by gear type")
    ...
    ps.add_argument("--gear", choices=list(tone3000.GEAR_VALUES), help="filter by gear type")
