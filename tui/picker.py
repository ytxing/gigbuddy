    ...
    async def _search_remote(self, query: str,
        ...
        if self._request_alive(generation, query):
            self.query_one("#pick-status", MarqueeBar).content = (
                "Searching TONE3000…")
        try:
            # The old RPC has no format parameter, so search the full catalog
            # then apply the canonical format predicate locally.  IR is a
            # format, not a gear filter; an IR tone may legitimately carry any
            # of the official gear values.  AMP must likewise exclude IR and
            # the non-NAM formats the current engine cannot load.
            rows = await asyncio.to_thread(
                library.tone3000.search,
                query, page_size=50, gear_filters=None)
            if self.kind == "ir":
                rows = [row for row in rows
                        if tone3000.tone_format(row) == "ir"]
            else:
                rows = [row for row in rows
                        if tone3000.tone_format(row) in (None, "nam")]
    ...
    def _handle_data(self, data: dict | None) -> None:
        ...
        if kind == "model":
            ...
            self.post_message(self.Picked(
                self.kind, model["local_path"], self.tone_type))
            self.dismiss()
            return
        if kind == "remote":
