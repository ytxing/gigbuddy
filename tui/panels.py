...
class PackFileTable(ClickSelectTable):
    ...
    def __init__(self) -> None:
        ...
        self.add_column("Pick", key="pick", width=4)
        self.add_column("Sel", key="sel", width=3)
        self.add_column("Format", key="format", width=10)
        self.add_column("Arch", key="arch", width=8)
        # REQ-019：不再显示 TONE id 列
        self.add_column("File", key="file", width=38)
        self.add_column("Size", key="size", width=10)
        self.configure_fixed_columns(
            {column.key.value: column.width for column in self.ordered_columns},
            minimums={"pick": 1, "sel": 1, "format": 3, "arch": 2,
                      "file": 10, "size": 4})
...
class DetailPane(Vertical):
    ...
    # Keep gear and format distinct.  Space is an IR-oriented gear but still
    # gets its own blue badge; pedal is the stable green action color.  The
    # fixed outboard violet remains visible even in themes whose warning color
    # is close to amp orange.
    _GEAR_STYLES = {
        "amp": "$primary",
        "cab": "$accent",
        "pedal": "$success",
    ...
    def _gear_badge(gear: str) -> str:
        gear = tone3000.normalize_gear(gear) or "unknown"
        if gear == "amp-cab":
    ...
    def _tone_summary(tone: dict, model_id: int | None = None) -> str:
        ...
        fmt = tone3000.tone_format(tone)
        if fmt:
            parts.append(f"[b $accent]{_escape(fmt.upper())}[/]")
    ...
    @staticmethod
    def _arch_tag(model: dict, colors: dict[str, str], tone: dict | None = None) -> str:
        """Render Architecture; IR is a Format, not an architecture."""
        if tone3000.is_ir_model(model, tone):
            return f"[b {colors['field']}]—[/]"
        version = tone3000.model_architecture_version(model)
        if version == "2":
            return f"[b {colors['header']}]A2[/]"
        if version == "custom":
            return f"[b {colors['section']}]CUSTOM[/]"
        return f"[b {colors['field']}]—[/]"
        ...
        for model in models:
            ...
            if local_path:
                ...
                try:
                    ...
                    size = None
                file_cell = f"[b]{_escape(ellipsis_window(filename, 34))}[/]"
                size_cell = (f"[dim]{self._fmt_size(size)}[/]" if size is not None
                             else "[dim]—[/]")
            else:
                # REQ-038：未下载标识 = 斜体 (not downloaded)，不用连字符。
                file_cell = (f"[dim]○ {_escape(ellipsis_window(filename, 30))} "
                             f"[i](not downloaded)[/i][/dim]")
                size_cell = "[dim]—[/]"
            is_ir = tone3000.is_ir_model(model, self._pack_tone)
            # A legacy amp-cab Tone can contain both NAM and IR files. Keep
            # the per-model format visible even when the parent says NAM.
            fmt = ("ir" if is_ir else tone3000.tone_format(self._pack_tone))
            if fmt is None and tone3000.model_architecture_version(model):
                fmt = "nam"
            format_cell = (fmt or "—").upper()
            table.add_row("\\[ ]", "·", format_cell,
                          self._arch_tag(model, colors, self._pack_tone),
    ...
    def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
        ...
        if getattr(self, "_pack_remote", False) or not model.get("local_path"):
            ...
            return
        if not tone3000.is_engine_compatible_model(model, tone):
            self.app.notify(
                "This model format is not supported by the current engine",
                severity="warning")
            return
        slot = "ir" if tone3000.is_ir_model(model, tone) else "model"
    ...
    def show_model(self, tone: dict | None, model: dict) -> None:
        ...
        if tone3000.is_ir_model(model, tone):
            ...
            version = tone3000.model_architecture_version(model)
            model_kind = {"2": "A2", "custom": "Custom"}.get(
                version, "MODEL")
        self._set_summary(
            f"[b $success]{_escape(model_kind)}[/] · "
