...
class LibraryPanel(Vertical):
    ...
    def on_mount(self) -> None:
        ...
        self._creator_suppressed_bottom_y: float | None = None
    ...
    def _show_status_if_empty(self, table: DataTable, message: str) -> None:
        ...
        # Keep Enter/r available after a failed worker. The tab click may have
        # left focus on the tab strip, while the status row is the retry target.
    ...
    def _maybe_load_more_from_viewport(self, table: LibraryTable, *, force: bool = False) -> None:
        ...
        if not has_more or loading:
            return
        if table.id == "lib-table-creators" and not force:
            suppressed_y = self._creator_suppressed_bottom_y
            if suppressed_y is not None:
                if abs(table.scroll_y - suppressed_y) < 1:
                    return
                self._creator_suppressed_bottom_y = None
    ...
    async def _load_more_creators(self) -> None:
        ...
        pinned_to_bottom = bool(
            table and table.max_scroll_y > 0
            and table.scroll_y >= table.max_scroll_y - 1)
        await self._show_top_creators(append=True)
        if pinned_to_bottom and table is not None:
            def pin_creator_bottom() -> None:
                self._creator_suppressed_bottom_y = table.max_scroll_y
                table.scroll_to(y=table.max_scroll_y, animate=False)
            self.call_after_refresh(pin_creator_bottom)
    ...
    def _row_cells(self, t: dict) -> list[str]:
        ...
        if state == "all":
            ...
            title = f"[dim]○[/] {title}"
        # A2 target count for amps; cabs expose their model count directly.
