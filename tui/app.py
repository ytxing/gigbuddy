...
class GigBuddyApp(App):
    ...
    /* Library/detail tables are deliberately single-axis surfaces. Their
       columns have fixed widths and long values are clipped in-cell; only
       rows should scroll. */
    LibraryPanel DataTable, DetailPane DataTable {
        overflow-x: hidden;
    }
    ...
    def _switch_chain_model(self, kind: str, direction: int) -> None:
        ...
        if not siblings:
            ...
            return
        tone = library.get_tone(siblings[0]["tone_id"]) or {}
        if kind == "cab":
            siblings = [m for m in siblings
                        if tone3000.is_ir_model(m, tone)]
        else:
            siblings = [m for m in siblings
                        if not tone3000.is_ir_model(m, tone)]
        if len(siblings) <= 1:
    ...
    def on_tone_selected(self, event: ToneSelected) -> None:
        ...
        if t:
            kind = "ir" if tone3000.is_ir_tone(t) else "amp"
