...
from .marquee import MarqueeBar, ellipsis_window
...
class PackInstallScreen(GigBuddyModal):
            ...
            with Horizontal(id="pack-split"):
                with Vertical(id="pack-left"):
                    ...
                    table = PackPickTable(id="pack-table", cursor_type="row",
                                          cell_padding=0)
                    table.add_column("✓", key="pick", width=4)
                    table.add_column("Model file", key="name", width=38)
                    table.add_column("Format", key="format", width=10)
                    table.add_column("Architecture", key="arch", width=8)
                    table.configure_fixed_columns(
                        {column.key.value: column.width
                         for column in table.ordered_columns},
                        minimums={"pick": 2, "name": 10, "format": 5,
                                  "arch": 4})
                    yield table
    ...
    def on_mount(self) -> None:
        ...
        t = self._tone
        self.run_worker(
            # The install table is the complete pack inventory.  Architecture
            # selection belongs to the user's row checkboxes, not this loader.
            partial(self._load_models, t["id"], False,
    ...
    async def _load_models(self, tone_id: int, a2_only: bool,
        ...
        try:
            ms = await asyncio.to_thread(
                tone3000.models, tone_id, a2_only=a2_only)
        ...
        self._selected = {m["id"] for m in ms
                          ...
                          and self._is_supported(m)}
        # A worker may finish during the short compose/unmount window.  The
        # generation check protects normal stale responses; this guard covers
        # the remaining Textual lifecycle race without surfacing a worker
        # exception to the whole app.
        try:
        ...
        for m in sorted(ms, key=lambda x: x["id"]):
            is_ir = tone3000.is_ir_model(m, self._tone)
            # A legacy amp-cab Tone can contain both NAM and IR files. The
            # model-level IR marker/path wins for this row's format label.
            fmt = ("ir" if is_ir else tone3000.tone_format(self._tone))
            if fmt is None and tone3000.model_architecture_version(m):
                fmt = "nam"
            format_label = (fmt or "—").upper()
            version = tone3000.model_architecture_version(m)
            # A1 is a legacy implementation detail.  Keep its file row and
            # selection behavior, but do not expose the deprecated tag.
            arch = {"2": "A2", "custom": "Custom"}.get(
                version, "—")
            checked = "\\[x]" if m["id"] in self._selected else "\\[ ]"
            table.add_row(checked, self._mark_name(m), format_label, arch,
                          key=str(m["id"]))
    ...
    def _is_supported(self, m: dict) -> bool:
        return tone3000.is_engine_compatible_model(m, self._tone)

    def _mark_name(self, m: dict) -> str:
        """行名：本地已下载的模型追加 ✓ downloaded 标记（u 的目标）。"""
        # Reserve room for the state suffix inside the fixed-width column.
        name = ellipsis_window(self._model_name(m), 30)
        if not self._is_supported(m):
            name += " [dim](unsupported format)[/dim]"
    ...
    def _toggle_row_key(self, row_key: str) -> None:
        ...
        if self._busy is not None:
            return
        mid = int(row_key)
        model = next((m for m in self._models if m["id"] == mid), None)
        if model is None:
            return
        if not self._is_supported(model):
            self._set_status("unsupported format — only NAM/IR can be loaded",
                             hint="ready")
            return
        self._uninstall_confirmed = False
    ...
    def action_toggle_all(self) -> None:
        ...
        self._uninstall_confirmed = False
        self._uninstall_target = []
        supported = {m["id"] for m in self._models if self._is_supported(m)}
        all_selected = self._selected == supported
        if all_selected:
    ...
    def _confirm(self) -> None:
        ...
        if self._busy is not None or not self._models:
            return
        supported = {m["id"] for m in self._models if self._is_supported(m)}
        sel = sorted(supported & self._selected)
        if not sel:
            fmt = tone3000.tone_format(t)
            if fmt is not None and fmt not in tone3000.ENGINE_FORMATS:
                self._set_status(
                    f"format {fmt.upper()} is not supported by the current engine",
                    hint="ready")
                return
            self._set_status(
