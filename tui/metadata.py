...
def _architecture_label(model: dict | None) -> str:
    """Display Model.architecture_version without calling IR an architecture."""
    model = model or {}
    version = tone3000.model_architecture_version(model)
    # Version 1 is a legacy implementation detail and is intentionally not a
    # user-facing architecture label.  IR remains represented by Format.
    return {"2": "A2", "custom": "Custom"}.get(version, "-")
...
def metadata_copy_text(tone: dict | None = None, model: dict | None = None,
    ...
    if tone:
        ...
        lines.append(f"Format: {_value(tone3000.tone_format(tone))}")
        if tone.get("tags"):
    ...
    if model:
        ...
        lines.extend([
            ...
            f"Architecture: {_value(_architecture_label(model))}",
        ])
...
def metadata_table(tone: dict | None = None, model: dict | None = None,
    ...
    def model_rows() -> None:
        ...
        row("FILE", "Architecture", _architecture_label(model))
        if model.get("model_url"):
    ...
    if tone:
        ...
        if not condensed:
            ...
            if author:
                ...
                row("IDENTITY", "Author", "?")
            row("IDENTITY", "Type", tone3000.normalize_gear(tone.get("gear")))
        row("IDENTITY", "Format", tone3000.tone_format(tone))
        ...
        if any(key in tone for key in (
            ...
            row("MODEL SET", "A2", tone.get("a2_models_count") or 0)
