...
def setup_v02_preset_state(monkeypatch, tmp_path):
    """Create a portable v0.2 project with two typed Slot files."""
    root = tmp_path
    ...
    monkeypatch.setattr("tui.app.live.CHAIN_FILE", root / "data" / "live_chain.json")
    ...
    with library.connect() as conn:
        library.upsert_tone(conn, {"id": 1, "title": "Test Rig", "gear": "amp-cab"},
                             commit=False)
        library.upsert_model(
            conn,
            {"id": 101, "tone_id": 1, "model_url": "amp", "name": "amp.nam",
             "architecture": "SlimmableContainer", "local_path": str(amp)},
            commit=False,
        )
        library.upsert_model(
            conn,
            {"id": 202, "tone_id": 1, "model_url": "cab", "name": "cab.wav",
             "architecture": "IR", "local_path": str(cab)},
            commit=False,
        )
        conn.commit()
    library.chain_set({
        "slots": [{"path": str(amp)}, {"path": None}, {"path": str(cab)}],
        "gain": 0.8,
        "master": 0.9,
        "quality": 0.7,
        "mute": False,
    ...
    return {
        ...
        "amp": amp,
        "cab": cab,
        "amp_rel": "data/tones/amp.nam",
        "cab_rel": "data/tones/cab.wav",
    }


def seed_v02_preset(name, chain, *, note="", active=False):
    """Insert a canonical v0.2 row and return its stable database identity."""
    with library.connect() as conn:
        conn.execute(
            "INSERT INTO presets (name, note, chain_json, created_at, updated_at) "
            "VALUES (?, ?, ?, '2026-08-06T00:00:00+00:00', ?)",
            (name, note, json.dumps(chain), f"2026-08-06T00:00:00+00:00-{name}"),
        )
...
def v02_chain(env, *, gain=0.8, master=0.9, quality=0.7):
    return {
        "slots": [
            {"model_id": 101, "path": env["amp_rel"]},
            {"model_id": None, "path": None},
            {"model_id": 202, "path": env["cab_rel"]},
        ],
        "gain": gain,
        "master": master,
        "quality": quality,
...
def test_preset_panel_s_saves_active_preset_with_slots(monkeypatch, tmp_path):
    env = setup_v02_preset_state(monkeypatch, tmp_path)
    library.chain_set({"slots": [{"path": str(env["amp"])}], "gain": 0.8,
                       "master": 0.9, "quality": 0.7})
    saved = library.preset_save("live", note="initial")
    library.chain_set({"slots": [{"path": str(env["cab"])}, {"path": None}],
                       "gain": 0.9, "master": 0.8, "quality": 0.6})
    ...
    async def scenario():
        ...
        async with app.run_test(size=(120, 40)) as pilot:
            ...
            table.focus()
            await pilot.pause()
            row_key = f"preset:{saved['id']}"
            assert table.ordered_rows[table.cursor_row].key.value == row_key

            await pilot.press("s")
            await pilot.pause()
            updated = library.preset_get("live")
            ...
            with library.connect() as conn:
                raw = json.loads(conn.execute(
                    "SELECT chain_json FROM presets WHERE name='live'"
                ).fetchone()[0])
...
def test_preset_panel_s_without_active_preset_opens_save_as(monkeypatch, tmp_path):
    env = setup_v02_preset_state(monkeypatch, tmp_path)
    ...
    async def scenario():
        ...
        async with app.run_test(size=(120, 40)) as pilot:
            ...
            assert library.preset_get("fresh")["chain"] == v02_chain(env)
...
def test_save_as_existing_name_requires_two_submits(monkeypatch, tmp_path):
    ...
    library.chain_set({"slots": [{"path": str(env["cab"])}], "gain": 0.9,
                       "master": 0.8, "quality": 0.6})
    ...
    async def scenario():
        ...
        async with app.run_test(size=(120, 40)) as pilot:
            ...
            await pilot.pause()
            assert isinstance(app.screen, PresetNameModal)
            assert library.preset_get("existing")["chain"] == {
                "slots": [{"model_id": 101, "path": env["amp_rel"]}],
                "gain": 0.8,
                "master": 0.9,
                "quality": 0.7,
            }
            ...
            await pilot.pause()
            assert library.preset_get("existing")["chain"] == {
                "slots": [{"model_id": 202, "path": env["cab_rel"]}],
                "gain": 0.9,
                "master": 0.8,
                "quality": 0.6,
            }
...
def test_preset_panel_keyboard_rename_edit_draft_and_delete(monkeypatch, tmp_path):
    env = setup_v02_preset_state(monkeypatch, tmp_path)
    library.preset_save("old", note="draft")
    ...
    async def scenario():
        ...
        async with app.run_test(size=(120, 40)) as pilot:
            ...
            before_live = library.chain_get()
            before_draft = library.preset_get("new")["chain"]
            await pilot.press("e")
            await pilot.pause()
            assert type(app.screen).__name__ == "PresetEditScreen"
            await pilot.press("+", "d", "alt+down", "alt+up")
            await pilot.pause()
            assert library.chain_get() == before_live
            assert library.preset_get("new")["chain"] == before_draft
...
def test_preset_panel_multi_select_bulk_delete(monkeypatch, tmp_path):
    env = setup_v02_preset_state(monkeypatch, tmp_path)
    one = seed_v02_preset("one", v02_chain(env))
    two = seed_v02_preset("two", v02_chain(env, gain=0.6))
    ...
    async def scenario():
        ...
        async with app.run_test(size=(120, 40)) as pilot:
            ...
            await pilot.press("a")
            await pilot.pause()
            assert app.query_one("PresetPanel")._selected == {
                f"preset:{one['id']}", f"preset:{two['id']}"
            }
            await pilot.press("d")
            await pilot.pause()
            await pilot.press("enter")
...
def test_preset_panel_table_is_bounded_and_scrollable(monkeypatch, tmp_path):
    ...
    for index in range(20):
        seed_v02_preset(f"preset-{index:02d}", v02_chain(env, gain=index / 20))
...
def test_preset_panel_double_click_loads_clicked_row(monkeypatch, tmp_path):
    env = setup_v02_preset_state(monkeypatch, tmp_path)
    older = seed_v02_preset("older", v02_chain(env))
    seed_v02_preset("newer", v02_chain(env, gain=0.6))

    async def scenario():
        app = GigBuddyApp(spawn_engine=False)
        async with app.run_test(size=(100, 32)) as pilot:
            table = app.query_one("#preset-table", DataTable)
            row_key = f"preset:{older['id']}"
            row = next(i for i, item in enumerate(table.ordered_rows)
                       if item.key.value == row_key)
            table.move_cursor(row=row, animate=False)
            await pilot.click(table, offset=(8, row + 1), times=2)
            await pilot.pause(0.4)
...
def test_preset_edit_draft_cancel_does_not_change_live_chain(monkeypatch, tmp_path):
    env = setup_v02_preset_state(monkeypatch, tmp_path)
    library.preset_save("draft", note="keep")
    ...
    async def scenario():
        ...
        async with app.run_test(size=(120, 40)) as pilot:
            ...
            before_live = library.chain_get()
            before_preset = library.preset_get("draft")
            await pilot.press("e")
            await pilot.pause()
            assert type(app.screen).__name__ == "PresetEditScreen"
            await pilot.press("+", "d", "alt+down", "alt+up")
            ...
            await pilot.pause()
            assert library.chain_get() == before_live
            assert library.preset_get("draft")["chain"] == before_preset["chain"]
            assert library.preset_get("draft")["note"] == "keep"
...
def test_preset_panel_enter_loads_the_focused_slot_snapshot(monkeypatch, tmp_path):
    """The v0.2 panel owns load; there is no secondary Preset Picker screen."""
    env = setup_v02_preset_state(monkeypatch, tmp_path)
    preset = seed_v02_preset("mayer-clean", v02_chain(env), note="Mayer 清音")
    loaded = []
    monkeypatch.setattr(
        "tui.presets.library.preset_load",
        lambda name: loaded.append(name) or
        {"slots": [{"path": str(env["amp"])}], "gain": 0.8,
         "master": 0.9, "quality": 0.7})
    monkeypatch.setattr("tui.presets.library.preset_resolved_chain", lambda name: {
        "slots": v02_chain(env)["slots"], "gain": 0.8,
        "master": 0.9, "quality": 0.7,
    })
...
def test_preset_save_writes_canonical_slots_and_relative_paths(monkeypatch, tmp_path):
    env = setup_v02_preset_state(monkeypatch, tmp_path)

    saved = library.preset_save("portable", note="ordered chain")

    assert saved["chain"] == v02_chain(env)
    with library.connect() as conn:
        raw = json.loads(conn.execute(
            "SELECT chain_json FROM presets WHERE name='portable'"
        ).fetchone()[0])
    assert set(raw) == {"slots", "gain", "master", "quality"}
    assert raw["slots"] == [
        {"model_id": 101, "path": env["amp_rel"]},
    ...
    stored_live = json.loads(library.CHAIN_FILE.read_text())
    assert [slot["path"] for slot in stored_live["slots"]] == [
        env["amp_rel"], None, env["cab_rel"]
    ]


def test_preset_panel_browse_and_load(monkeypatch, tmp_path):
    """Preset panel lists ordered Slots; highlight previews them, Enter loads."""
    env = setup_v02_preset_state(monkeypatch, tmp_path)
    ...
    monkeypatch.setattr("tui.presets.library.preset_resolved_chain", lambda name: {
        "slots": [
            {"model_id": 101, "path": str(env["amp"])},
            {"model_id": None, "path": None},
            {"model_id": 202, "path": str(env["cab"])},
        ],
        "gain": 0.8,
        "master": 0.9,
        "quality": 0.7,
    ...
    async def scenario():
        ...
        async with app.run_test(size=(120, 40)) as pilot:
            ...
            row_key = f"preset:{preset['id']}"
            assert table.ordered_rows[table.cursor_row].key.value == row_key
            column_keys = {column.key.value for column in table.columns.values()}
            assert "slots" in column_keys
            column_labels = {str(column.label) for column in table.columns.values()}
            assert "Slots" in column_labels
            summary = str(table.get_cell(row_key, "slots"))
            ...
            assert "quality" in body and "0.70" in body
            ...
            # enter → loads the complete Slot snapshot
            await pilot.press("enter")
            await pilot.pause()
            assert loaded == ["mayer-clean"]
