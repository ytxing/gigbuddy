...
def test_existing_schema_gets_semantic_name_column():
    ...
    with library.connect() as conn:
        ...
        assert {"format", "sizes", "license", "links", "user", "is_public", "url"} <= tone_columns
        model_columns = {row[1] for row in conn.execute("PRAGMA table_info(models)")}
        assert {"created_at", "updated_at", "user_id", "size"} <= model_columns
...
def test_mark_download_state(monkeypatch, tmp_path):
    ...
    remote_models = {
        19: [{"id": 1001, "architecture": "SlimmableContainer"},
             {"id": 9000, "architecture": "WaveNet"}],             # A1, not local
    ...
    assert by_id[19]["download_state"] == "partial"  # A1 is also applicable
