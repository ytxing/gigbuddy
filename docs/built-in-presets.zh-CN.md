...
正常安装时，`scripts/bootstrap.py` 会在 TUI 打开前准备内置 preset 所需的远程
模型。启动 GigBuddy 时再完成一次很快的本地注册：

1. 读取仓库中的 JSON，把名称和音色链登记到本地 SQLite 索引。
2. 安装器按 TONE3000 Tone 合并缺少的模型，通过正常校验路径下载。
3. 在 **PRESETS** 面板显示全部内置行；注册过程不会改动当前正在使用的
   Tone Chain。
4. 如果使用了 `--skip-presets`，或安装期间某些远程下载失败，相关行会保持
   `UNAVAILABLE`；加载该行时只针对这一套 preset 重试。

内置 JSON 如果格式损坏或内部不一致，则不属于可重试的远程失败；安装器会明确
报错，不会把这套目录当作完整可用来结束安装。

状态含义如下：

| 状态 | 含义 |
|---|---|
| `PREPARING` | 正在针对该 preset 重试下载缺少的一个或多个模型。 |
...
没有被删除。正常安装会登记内置目录，并在 TUI 打开前等待 20 套音色所需模型的
准备完成。个别远程下载失败时，安装仍可完成，相关行保留为 `UNAVAILABLE`，之后
可以重试。
