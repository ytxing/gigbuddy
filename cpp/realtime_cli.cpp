...
static void apply_chain(const nlohmann::json& j, NeuralAudio::NeuralModelLoader& loader,
    ...
    if (j.contains("model") && j["model"].is_string()) {
        ...
        if (p != ctx.liveModelPath) {
            ...
            if (nm) { std::atomic_store(&ctx.model, nm); ctx.liveQuality = 1.0f; fprintf(stderr, "[live] 模型 -> %s\n", p.c_str()); }
...
static int pa_callback(const void* in, void* out, unsigned long frames,
    ...
    if (c->inputGain != 1.0f)
        for (unsigned long i = 0; i < frames; i++) c->scratch[i] *= c->inputGain;
    c->inPeak.store(block_peak(mono, (int)frames), std::memory_order_relaxed);
    auto m = std::atomic_load(&c->model);
    if (m) m->Process((float*)mono, c->scratch.data(), (int)frames);
    else std::fill(c->scratch.begin(), c->scratch.begin() + frames, 0.0f);
    auto f = std::atomic_load(&c->fir);
    if (f) f->process(c->scratch.data(), outBuf, (int)frames);
    else std::copy(c->scratch.begin(), c->scratch.begin() + frames, outBuf);
    float mstr = c->master.load(std::memory_order_relaxed);
    if (mstr != 1.0f)
        for (unsigned long i = 0; i < frames; i++) outBuf[i] *= mstr;
